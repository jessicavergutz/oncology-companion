...
import { ad6ScenesData } from '../../data/bussola/bussola-ad6-data';
...
/* ───────────────────────────────────────────
   MAIN COMPOSITION
   ─────────────────────────────────────────── */
export const BussolaAd6 = () => {
	const frame = useCurrentFrame();
	const { durationInFrames } = useVideoConfig();
	const CROSSFADE = 15;

	// Audio volume automation (fade in/out)
	...
	return (
		<AbsoluteFill style={{ backgroundColor: '#643A1F' }}>
			...
			<Audio src={staticFile('audio/ad6.mp3')} volume={volume} />

			{ad6ScenesData.map((scene, index) => {
				const isLast = index === ad6ScenesData.length - 1;
