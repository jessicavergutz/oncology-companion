...
// Import data strictly from the data block
import { scenesData } from '../../data/bussola/bussola-ad-30s-data';
...
const CinematicScene = ({ scene }: { scene: any }) => {
	...
	return (
		<AbsoluteFill style={{ opacity }}>
			...
			<AbsoluteFill>
				<Img
					// Must reference images from src/assets via static file for now or relative import
					// But we will use absolute path import here since Remix requires relative imports 
					// Actually, using require/import is better than staticFile for assets inside src.
...
/* ───────────────────────────────────────────
   MAIN COMPOSITION
   ─────────────────────────────────────────── */
export const BussolaAd30s = () => {
	const CROSSFADE = 15;
	return (
		<AbsoluteFill style={{ backgroundColor: '#643A1F' }}>
			<Audio src={staticFile('audio/bussola-voice.mp3')} />
			{scenesData.map((scene, index) => {
				const isLast = index === scenesData.length - 1;
